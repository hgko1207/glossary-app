const AbbrManager = function() {
	...
	const DataTable = {
			...
			columns: [
				...
				{ data: "name" },
				{
					...
					render: function(data, type, row, meta) {
						...
						+ 'onClick="AbbrManager.modal(' + row.id + ')"><i class="icon-pencil7"></i></button>'
						+ '<button type="button" class="btn btn-outline bg-danger text-danger-600 btn-sm" '
						+ 'onClick="AbbrManager._delete(' + row.id + ')"><i class="icon-trash"></i></button>';
					}
				}
				]
		},
		...
		search: function() {
			...
			Datatables.rowsAdd(this.table, contextPath + "/abbr/search", param);
		}
	}
	...
	return {
		...
		_delete: function(id) {
			deleteCommon(contextPath + "/abbr/delete", id, name, DataTable);
		},
		modal: function(id) {
			$.ajax({
	    		url: contextPath + "/abbr/get",
	    		type: "GET",
	    		data: {"id": id},
